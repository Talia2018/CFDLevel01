...
*/
    ...
    /* 默认无登录 */
    default:
    $this->error("Invalid third party method",U('/user/login/index'));
    break;
    }
    }  
      /*    
 ...
 public function callback() {
    ...
    /* 接受QQ互联登录返回值 */
    case qq:
    empty($_GET['code']) && $this->error("Invalid third party method",U('/user/login/index'));
    $app_id = C('SZ_QQ_APP_ID');
    ...
    $back = $qq->callback($app_id , $app_key, $callback);
                        //防止刷新
    empty($back) && $this->error("Please re-authorize login",U('/user/login/index'));
