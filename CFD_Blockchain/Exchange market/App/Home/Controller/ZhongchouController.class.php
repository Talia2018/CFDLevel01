...
class ZhongchouController extends CommonController {
	...
	public function details(){
		...
		$list=M('Issue')
			...
			->where('id='.$_GET['id'])
			->find();
		$list['buy_name'] = $list['buy_currency_id']==0?"USTD":M('Currency')->field('currency_name')->where("currency_id='{$list['buy_currency_id']}'")->find()['currency_name'];
		$list['buy_mark'] = $list['buy_currency_id']==0?"USTD":M('Currency')->field('currency_mark')->where("currency_id='{$list['buy_currency_id']}'")->find()['currency_mark'];
		...
		if (!empty($uid)){
			if($list['buy_currency_id']!=0){
				...
				$buy_num = M('Member')->field('USTD')->where("member_id=$uid")->find();
				$buy_num = $buy_num['USTD'];
			}
		}
...
//    	$member=$this->member;
		...
		if($issue['limit_count']<=$count){
			$data['status']=0;
			$data['info']='The number of approvals cannot exceed the number of purchases';
		...
		if($issue['min_limit']>$num){
			$data['status']=0;
			$data['info']='The number of approvals cannot be less than the minimum number of approvals';
			$this->ajaxReturn($data);
		}
		if($issue['deal']<$num){
			$data['status']=0;
			$data['info']='The number of approvals cannot exceed the remaining quantity';
			$this->ajaxReturn($data);
		}
		if($issue['limit']<$num_buy+$num){
			$data['status']=0;
			$data['info']='You have exceeded the total approved purchase limit';
			$this->ajaxReturn($data);
		}
		if($buy_num<$num*$issue['price']){
			$data['status']=0;
			$data['info']='Your account balance is insufficient';
			$this->ajaxReturn($data);
		}
		//修改会员表USTD字段
//    	$rs=M('Member')->where("member_id=$uid")->setDec('USTD',$num*$issue['price']*$config['bili']);
		if($buy_currency_id!=0){
			$rs = M('Currency_user')->where("member_id=$uid and currency_id={$issue['buy_currency_id']}")->setDec('num',$num*$issue['price']);
		}else{
			$rs = M('Member')->where("member_id=$uid")->setDec('USTD',$num*$issue['price']);
		}
		//添加财务日志表
		if($rs){
			//添加认购记录
			...
			if($issue['is_forzen']==0){
				...
				$this->addFinance($uid, 8, 'Crowdfunding deduction'.$num*$issue['price'], $num*$issue['price'], 2, $buy_currency_id);
				$this->addFinance($uid, 9, 'Crowdfunding'.$num, $num, 1, $issue['currency_id']);
			//添加信息表
			$this->addMessage_all($uid, -2, 'Successful subscription', 'The crowdfunding project '.$issue['title'].' you participated in has been successful, deducting the transaction currency '.$num*$issue['price'].', and getting the crowdfunding currency'.$num);
			//添加众筹推荐人员奖励
			...
			$data['status']=0;
			$data['info']='Recruitment success';
			$this->ajaxReturn($data);
		}else{
			//添加信息表
			$this->addMessage_all($uid, -2, 'Crowdfunding failed', 'The crowdfunding project you participated in'.$issue['title'].'Unsuccessful');
			$data['status']=0;
			$data['info']='Crowdfunding failed';
			$this->ajaxReturn($data);
		}
	}
	...
	public function setAwards($arr,$uid,$num){
		...
		if ($one_info['status']==1&&$u_info['is_lock']==0){//只有用户正常情况才会有推荐奖励
    		  ...
    		  $this->addFinance($one_info['member_id'], 12, 'Crowdfunding recommendation reward'.$num, $num, 1, $arr['zc_awards_currency_id']);
		}
		...
		if ($two_info['status']==1&&$two_info['is_lock']==0){//只有用户正常情况才会有推荐奖励
    		...
    		$this->addFinance($two_info['member_id'], 12, 'Crowdfunding recommendation reward'.$num, $num, 1, $arr['zc_awards_currency_id']);
		}
		if($r){
			return true;
		}
