...
class TradeController extends TradeFatherController {
    ...
    public function buy(){
        if(!$this->checkLogin()){
            $data['status']=0;
            $data['info']='Please log in before doing this';
        ...
        $over_time=$time+$this->config['jiaoyi_over_hour']*3600+$this->config['jiaoyi_over_minute']*60;
   		if(time()<$start_time||time()>$over_time){
			$data['status']=-10;
			$data['info']='The transaction is not open, please trade within the trading hours.';
			$this->ajaxReturn($data);
		}
        ...
        if ($currency['is_lock']){
            $data['status']=-5;
            $data['info']= 'The currency is temporarily not allowed to trade';
            $this->ajaxReturn($data);
        }
        
        if(!is_numeric($buyprice)||!is_numeric($buynum)){
            $data['status']=0;
            $data['info']='Your order price or quantity is incorrect, please modify';
            $this->ajaxReturn($data);
        }
        //涨停价格限制
        if ($currency['price_down']>0&&$buyprice<$currency['price_down']){
            $msg['status']=-9;
            $msg['info']='The transaction price exceeds the limit price limit';
            $this->ajaxReturn($msg);
        }
        ...
        if ($currency['price_up']>0&&$buyprice>$currency['price_up']){
            $msg['status']=-7;
            $msg['info']='Transaction price exceeds the daily limit price limit';
            $this->ajaxReturn($msg);
        }
        
        if ($buyprice*$buynum<1){
            $data['status']=0;
            $data['info']='Can not order orders below 1';
            $this->ajaxReturn($data);
        }
        if (!is_int($buynum)){
            $data['status']=-1;
            $data['info']='The number of transactions must be an integer';
            $this->ajaxReturn($data);
        }
        if ($buynum<0){
            $data['status']=-2;
            $data['info']='The number of transactions must be positive';
            $this->ajaxReturn($data);
        }
        $member=$this->member;
        if(md5(I('post.buypwd'))!=$member['pwdtrade']){
            $data['status']=-3;
            $data['info']='Transaction password is incorrect';
            $this->ajaxReturn($data);
        }
 ...
 //      M()->query("UNLOCK TABLES");
       if (in_array(false, $r)){
           M()->rollback();
           $msg['status']=-7;
           $msg['info']='Operation not successful';
           $this->ajaxReturn($msg);
       }else {
           M()->commit();
           $msg['status']=1;
           $msg['info']='Successful operation';
           $this->ajaxReturn($msg);
       }
	...
	public function sell(){
	   ...
	   if ($currency['is_lock']){
	       $msg['status']=-2;
	       $msg['info']='The currency is temporarily unavailable';
	       $this->ajaxReturn($msg);
	   }
	   
		if (empty($sellprice)||empty($sellnum)){
		    $msg['status']=-3;
		    $msg['info']='Sell price or quantity cannot be empty';
		    $this->ajaxReturn($msg);
		}
		...
		if (empty($sellpwd)){
		    $msg['status']=-4;
		    $msg['info']='Transaction password cannot be empty';
		    $this->ajaxReturn($msg);
		}
		if ($this->member['pwdtrade']!=md5($sellpwd)){
		    $msg['status']=-5;
		    $msg['info']='Transaction password is incorrect';
		    $this->ajaxReturn($msg);
		}
		//涨停价格限制
		...
		if ($this->checkUserMoney($sellnum, $sellprice, 'sell', $currency)){
		    $msg['status']=-6;
		    $msg['info']='Your account balance is insufficient';
		    $this->ajaxReturn($msg);
		}
		//减可用钱 加冻结钱
		M()->startTrans();
        ...
        switch ($type){
            ...
            case 'buy':
               ...
               $r[]=$this->setUserMoney($this->member['member_id'],$order['currency_trade_id'], $trade_num*($order['price']-$trade_price), 'inc', 'num');
                ...
                //手续费
                $r[]=$this->addFinance($order['member_id'], 11, 'Transaction Fees',$trade_num*($order['fee']/100), 2, $order['currency_id']);
                $r[]=$this->addFinance($trade_order['member_id'], 11, 'Transaction Fees',$trade_num*$trade_price*($trade_order['fee']/100), 2, $order['currency_trade_id']);
                break;
            case 'sell':
                ...
                //手续费
                $r[]=$this->addFinance($order['member_id'], 11, 'Transaction Fees',$trade_num*$trade_price*($order['fee']/100), 2, $order['currency_trade_id']);
                $r[]=$this->addFinance($trade_order['member_id'], 11, 'Transaction Fees',$trade_num*($trade_order['fee']/100), 2, $order['currency_id']);
                break;
        }
        //修正最终成交的价格
