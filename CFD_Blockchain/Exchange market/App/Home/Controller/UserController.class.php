...
class UserController extends HomeController {
        ...
        if(IS_POST){
            ...
            if($data['nick']!=$list['nick']){
                ...
                if($M_member->field('nick')->where($where)->select()){
                    $data['status'] = 2;
                    $data['info'] = 'Nickname repeat';
                    $this->ajaxReturn($data);
                }
            }
            if(empty($data['province'])){
                $data['status'] = 2;
                $data['info'] = 'Please fill in the province/State';
                $this->ajaxReturn($data);
            }
            if(empty($data['city'])){
                $data['status'] = 2;
                $data['info'] = 'Please fill in the city';
                $this->ajaxReturn($data);
            }
    ...
    public function updatePassword(){
        ...
        if(IS_POST){
            ...
            if(!$M_member->checkPwd($_POST['oldpwd']) || !$M_member->checkPwd($_POST['pwd']) || !$M_member->checkPwd($_POST['repwd']) ){
                $data['status']=2;
                $data['info']='Please enter a 6-20 digit password';
                $this->ajaxReturn($data);
            }
            if($rePwd!=$newPwd){
                $data['status']=2;
                $data['info']='Inconsistent password entered twice';
                $this->ajaxReturn($data);
            }
            $r = $M_member->where(array('member_id'=>session('USER_KEY_ID'),'pwd'=>$oldPwd))->find();
            if(!$r){
                $data['status']=2;
                $data['info']='Original password input error';
                $this->ajaxReturn($data);
            }
            if($newPwd===$oldPwd){
                $data['status']=2;
                $data['info']='The new password cannot be the same as the password';
                $this->ajaxReturn($data);
            }
            $data['pwd'] = $newPwd;
            ...
            if(!$r){
                $data['status']=2;
                $data['info']='Server is busy, please try again later';
                $this->ajaxReturn($data);
            }
            $data['status']=1;
            $data['info']='Successfully modified..please login again';
            session_destroy();
            $this->ajaxReturn($data);
        }else{
            $this->User_status();
    ...
    public function rmbChangeCurrency($rmb=0,$bili=1,$currency_id){
		...
		if(!empty($r)){
			$this->success('Successful trading');
		}else{
			$this->error('Failed trading');
		}
    }
    /**
     * 修改支付密码
    ...
    public function updatePwdTrade(){
        if(IS_POST){
            ...
            if($Examine){
                $this->error("You have already submitted and are under review..");
                return;
            }
            if(!checkPwd($data['pwd'])){
                $this->error("Password input digits are incorrect");
                return;
            }
            if(!checkPwd($oldpwdtrade)){
                $this->error("Transaction Password input digits are incorrect");
                return;
            }
            if(!checkPwd($data['pwdtrade'])){
                $this->error("New Password input digits are incorrect");
                return;
            }
            if($data['pwdtrade'] != $repwdtrade){
                $this->error("Two payment password input inconsistencies");
                return;
            }
            ...
            if($info['pwdtrade']!=md5($oldpwdtrade)){
                $this->error("Original payment Incorrect password");
                return;
            }
            if($info['pwd']==md5($data['pwdtrade'])){
                $this->error("The payment password cannot be the same as the login password.");
                return;
            }
            ...
            if(empty($info['pic_1'])){
                $this->error('Picture 1'.$upload->getError());
                return;
            }
            if(empty($info['pic_2'])){
                $this->error('Picture 2'.$upload->getError());
                return;
            }
            if(empty($info['pic_3'])){
                $this->error('Picture 3'.$upload->getError());
                return;
            }
            ...
            $this->success('Successful application, notify you by system notification after review',U('User/index'));
        }
    }
    /**
     * 邀请好友
            ...
            if($list){
                foreach ($list as $k=>$vo) {
                    if(M('Member')->where(array('email'=>$vo))->find()){
                        $data['status'] = 0;
                        $data['info'] = "The ".$vo." email you entered is already registered.";
                        $this->ajaxReturn($data);
                    }else{
                        $arr[] = $vo;
                    }
                ...
                foreach ($arr as $vo) {
                    ...
                    $content = "<div>";
                    $content.= "Hello,<br><br>Please click on the link:<br>";
                    $content.= "<a target='_blank' href='{$url}' >Complete registration invitation</a>";
                    $content.= "<br><br>If the link cannot be clicked, please copy and open the following URL:<br>";
                    $content.= "<a target='_blank' href='{$url}' >{$url}</a>";
                    if(setPostEmail($this->config['EMAIL_HOST'],$this->config['EMAIL_USERNAME'],$this->config['EMAIL_PASSWORD'],$this->config['name'].'team',$vo,$this->config['name'].'team[Registration invitation]',$content)){
                        $data['status']=0;
                        $data['info']="Email ".$vo." failed to send";
                        $this->ajaxReturn($data);
                    }
                }
                $data['status']=1;
                ...
                $data['status'] = 0;
                $data['info'] = "Please enter the sending email address";
                $this->ajaxReturn($data);
            }
        }
        //我的邀请
//         $my_invit = M('Member')->field('email,status,reg_time')->where(array('pid'=>session('USER_KEY_ID')))->select();
        ...
        foreach ($my_invit as $k=>$vo) {
        	$my_invit[$k]['status_name'] = $vo['status']?"Filled in personal information ":" has not filled in personal information";
        }
...
//        $this->assign('after',$after);
    ...
    public function insert(){
        ...
        if(IS_POST){
			...
			if(empty($info['bname'])){
                $data['status']  = 0;
                $data['info'] = 'Please fill in the label';
                $this->ajaxReturn($data);
            }
            if(empty($info['bankname'])){
                $data['status']  = 2;
                $data['info'] = 'Please select a bank';
                $this->ajaxReturn($data);
            }
            if(empty($info['address'])){
                $data['status']  = 3;
                $data['info'] = 'Please select an account opening address';
                $this->ajaxReturn($data);
            }
            if(empty($info['bank_branch'])){
            	$data['status']  = 7;
            	$data['info'] = 'Please choose an account branch';
            	$this->ajaxReturn($data);
            }
            if(empty($info['cardnum'])){
                $data['status']  = 4;
                $data['info'] = 'Please enter the bank card number';
                $this->ajaxReturn($data);
            }
            if(16 > strlen(I("account")) || strlen(I("account")) > 19){
            	
                $data['status']  = 5;
                $data['info'] = 'Please enter a valid bank card number';
                $this->ajaxReturn($data);
            }
            $re = $bank->add($info);
            if($re>0){
                $data['status']  = 1;
                $data['info'] = 'Successful operation';
                $this->ajaxReturn($data);
            }else{
                $data['status']  = 6;
                $data['info'] = 'The server is busy, please try again later';
    ...
    public function delete(){
        ...
        if($count != 0){
        	$arr['status']=-1;
        	$arr['info']="The address still has a withdrawal record and cannot be deleted!";
        	$this->ajaxReturn($arr);
        }
        ...
        if($re){
            $arr['status']=1;
            $arr['info']="Successful operation";
            $this->ajaxReturn($arr);
        }else{
            $arr['status']=0;
            $arr['info']="The server is busy, please try again later";
            $this->ajaxReturn($arr);
        }
    }
    ...
    public function withdraw(){
        ...
        //查找member_id对应的Transaction 密码
        $where['member_id'] = session('USER_KEY_ID');
        //查找member表uid对应信息（Transaction 密码，可以金额，冻结金额）
        ...
        if(IS_POST){
            ...
            $data['pwdtrade'] = md5(I('post.pwdtrade'));
           	if($_POST['code']!=$_SESSION['code']){
           		$info['status']=11;
           		$info['info']='Incorrect verification code';
           		$this->ajaxReturn($info);
           	}
            if(empty($data['all_money'])){
                $info['status'] = 0;
                $info['info'] = "Please fill in the amount of cash";
                $this->ajaxReturn($info);
            }
            //单笔在100至50000在之间
            if($data['all_money']<10||$data['all_money']>500000){
                $info['status'] = 2;
                $info['info'] = "Withdrawal exceeds limit";
                $this->ajaxReturn($info);
            }
            //单日是否超出50W限制
            ...
            if($res == false){
                $info['status'] = 3;
                $info['info'] = "The amount of cash raised this time exceeds the maximum amount of cash raised in a single day.";
                $this->ajaxReturn($info);
            }
            //验证密码
            if(empty($data['pwdtrade'])){
                $info['status'] = 4;
                $info['info'] = "Please fill in the Transaction password";
                $this->ajaxReturn($info);
            }
            if($data['pwdtrade'] != $mem_data['pwdtrade']){
                $info['status'] = 5;
                $info['info'] = "Transaction password is incorrectly filled in";
                $this->ajaxReturn($info);
           }
            ...
            if(empty($data['bank_id'])){
                $info['status'] = 6;
                $info['info'] = "Please select the withdrawal address";
                $this->ajaxReturn($info);
            }
            
            if($data['all_money']>$mem_data['rmb']){
            	$info['status'] = 6;
            	$info['info'] = "Insufficient account balance";
            	$this->ajaxReturn($info);
            }

            if($mem_data['rmb']<100){
                $info['status'] = 7;
                $info['info'] = "Cash less than 100, can't withdraw cash";
                $this->ajaxReturn($info);
            }
            //应付手续费
            ...
            if($re){
                if($res){
                    $info['status'] = 1;
                    $info['info'] = "Withdrawal success, arrival within 24 hours ";
                    $this->ajaxReturn($info);
                }else{
                    $info['status'] = 8;
                    $info['info'] = "The server is busy, please try again later";
                    $this->ajaxReturn($info);
                }
            }else{
                $info['status'] = 9;
	...
	public function chexiaoByid(){
		...
		if($rmb['forzen_rmb'] < 0){
			$data['status'] = 4;
			$data['info'] = "Undo failed, the amount of the freeze is incorrect, please contact the administrator";
			$this->ajaxReturn($data);
		}
		//查状态是否在可操作的状态
		if($money['status']!=3){
			$data['status'] = 5;
			$data['info'] = "Do not repeat";
			$this->ajaxReturn($data);
		}
		//加回可用金额
		...
		if(!$res){
			...
			$data['status'] = 1;
			$data['info'] = "Successful cancellation";
			$this->ajaxReturn($data);
	}
