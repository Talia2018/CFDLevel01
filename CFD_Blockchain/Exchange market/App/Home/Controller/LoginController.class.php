...
class LoginController extends CommonController{
    ...
    public function checkLog(){
        ...
        if((checkEmail($email) || checkMobile($email))==false){
            $data['status']=2;
            $data['info']="Please enter the correct phone or email address";
        ...
        if($info['status']==2){
            $data['status']=2;
            $data['info']="Sorry that your account has been disabled.";
            $this->ajaxReturn($data);
        }
        //验证手机或邮箱
        if($info==false){
            $data['status']=2;
            $data['info']="email or mobile phone does not exist";
            $this->ajaxReturn($data);
        }
        //验证密码
        if($info['pwd']!=$pwd){
        ...
        if($old_login_ip != $new_ip && $info['idcard'] ){
            if($card != $idcard){
                $data['status']=2;
                $data['info']="Birthday does not match your current fill";
                $this->ajaxReturn($data);
            }
        }
//        $this->pullMessage($info['member_id'],$info['login_time']?$info['login_time']:$info['reg_time'] );
        if($this->pullMessage($info['member_id'],$info['login_time']?$info['login_time']:$info['reg_time'])==false){
            $data['status']=2;
            $data['info']="The server is busy, please try again later12";
            $this->ajaxReturn($data);
        }
    ...
    public function qqLogin(){
    	...
    	//防止刷新
    	empty($back) && $this->error("Please re-authorize login",U('Login/index'));
    	...
    	if($MemberArray['member_id']!=""){
    		...
    		$this->error("Login successfully",U('Index/index'));
    		...
    		if($Member->create($add)){
    			...
    			if($userid){
    				...
    				$this->error("Login failed",U('Login/index'));
    			}
    		}
    	}
    }
    ...
    public function findpwd(){
        if(IS_AJAX){
            if(empty($_POST['email'])){
                $data['status']=2;
                $data['info']="please enter your email";
                $this->ajaxReturn($data);
            }
            if(!checkEmail($_POST['email'])){
                $data['status']=2;
                $data['info']="please enter your vaild email";
                $this->ajaxReturn($data);
            }
            if(empty($_POST['captcha'])){
                $data['status']=2;
                $data['info']="Please fill in verification code";
                $this->ajaxReturn($data);
            }
            $verify = new Verify();
            if(!$verify->check($_POST['captcha'])){
                $data['status']=2;
                $data['info']="Incorrect verification code";
                $this->ajaxReturn($data);
            }
            ...
            $content.= "<a target='_blank' href='{$url}' >Reset your password</a>";
            ...
            $r = setPostEmail($this->config['EMAIL_HOST'],$this->config['EMAIL_USERNAME'],$this->config['EMAIL_PASSWORD'],$this->config['name'].'Team', $_POST['email'], $this->config['name'].'Team [password retrieved]',$content);
	        if($r){
            	$data['status']=2;
            	$data['info']="The server is busy, please try again later";
            	$this->ajaxReturn($data);
    ...
    public function resetPwd(){
        ...
        if(IS_POST){
            ...
            if(empty($_POST['pwd'])){
                $data['status']=2;
                $data['info']="Please enter your password";
                $this->ajaxReturn($data);
            }
            if(!checkPwd($_POST['pwd'])){
                $data['status']=2;
                $data['info']="Password length is between 6-20 characters";
                $this->ajaxReturn($data);
            }
            if($_POST['repwd'] != $_POST['pwd']){
                $data['status']=2;
                $data['info']="Confirm password and password are inconsistent";
                $this->ajaxReturn($data);
            }
    ...
    public function checkIp($email){
        ...
        if(!checkEmail($email) && !checkMobile($email)){
            $data['status'] = 2;
            $data['msg'] = 'Please enter your vaild email or mobile number';
            $this->ajaxReturn($data);
        }
        if(checkEmail($email)){
            $where['email']  = $email;
        ...
        if($info['idcard']){
            ...
            if($old_login_ip!=$new_ip){
                $data['status'] = 1;
                $data['msg'] = 'The system has detected that your account login IP is different from the previous one. In order to protect your account assets, please enter your birth date on the reserved ID of '.$this->config['name'].' If you do not have real-name certification, please contact customer service certification.';
                $this->ajaxReturn($data);
            }
        }
        $data['status'] = 0;
