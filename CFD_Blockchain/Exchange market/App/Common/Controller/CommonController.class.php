...
class CommonController extends Controller {
	...
	protected function  User_status(){
		...
		if($list['status']==0){
			session('STATUS',0);
			$this->redirect('ModifyMember/modify','',1,"<script>alert('Please fill in your personal information')</script>");
			exit();
		}
	}
...
//      }
     ...
     /**
      * 判断用户进入Earning bonus shares数量是否超过当日上线
      * @param $money 用户表dividend_num代表分红业绩与金额等比例
      * @param
      * return 还可以获得的分红股数
      */
     ...
     private function addDividend($member_id,$num,$max){
     	...
     	if($n){
     		...
     		$this->addFinance($member_id, 13, "Earning bonus shares",$money*$n,1, $this->dividendConfig['dividend_id']);
