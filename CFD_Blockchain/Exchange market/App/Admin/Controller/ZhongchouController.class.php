...
class ZhongchouController extends AdminController {
	...
	private function wenjianDownload($file){
		...
		if(!$info) {
			...
			if(isset($url)){
				...
				$this->error('File upload failed');exit();
			}
		}
	}
    ...
    public function jiedongById(){
    	...
    	if($log['status']==1){
    		$this->error('The record has been processed');
    	}
    ...
    public function jiedong_start(){	
    	...
    	if(!$log){
    		$data['status']=0;
    		$data['info']='No corresponding record';
    ...
    public function log(){
        ...
        foreach ($log as $key=>$vo) {
            $log[$key]['buy_name'] = $vo['buy_currency_id']==0?"USDT":M('Currency')->where("currency_id='{$vo['buy_currency_id']}'")->find()['currency_name'];
        }
	...
	public function awards(){
		...
		if(IS_POST){
			if(empty($_POST['zc_awards_currency_id'])){
				$this->error('Please fill in the currency');
				return;
			}
			if(!isset($_POST['zc_awards_one_ratio'])){
				$this->error('Please fill in the first level');
				return;
			}
			if(!isset($_POST['zc_awards_two_ratio'])){
				$this->error('Please fill in the second level');
				return;
			}
			if(!isset($_POST['zc_awards_status'])){
				$this->error('Please fill in the open status');
				return;
			}
			if($_POST['zc_awards_one_ratio']>100 || $_POST['zc_awards_two_ratio'] > 100){
				$this->error('The ratio cannot be greater than 100%');
				return;
			}
