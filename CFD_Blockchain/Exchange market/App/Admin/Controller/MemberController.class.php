...
class MemberController extends AdminController {
    ...
    public function addMember(){
        if(IS_POST){
            ...
            if($r = $M_member->create()){
                if($r['pwd']==$r['pwdtrade']){
                    $this->error('The payment password cannot be the same as the password');
                    return;
                }
                if($M_member->add($r)){
                    $this->success('Added successfully',U('Member/index'));
                    return;
                }else{
    ...
    public function show_my_invit(){
        ...
        if($my_invit){
            ...
            $this->error('Sorry, you have not recommended others yet.');
            return;
        }
    }
    /**
    ...
    public function saveMember(){
        ...
        if(IS_POST){
            ...
            if($_POST['pwd'] == $_POST['pwdtrade'] && $_POST['pwd']!=null ){
                $this->error('The transaction password cannot be the same as the password');
                return;
            }
            ...
            if($_POST['phone']!=$list['phone']){
                ...
                if($M_member->field('phone')->where($where)->select()){
                    $this->error('Mobile phone number repeat');
                    return;
                }
            }
            ...
            if($r!==false){
                $this->success('Successfully modified',U('Member/index'));
                return;
            }else{
    ...
    public function delMember(){
        ...
        if($member['rmb']>0||$member['forzen_rmb']>0||$member_currency['num']>0||$member_currency['forzen_num']>0){
            $this->error('It is forbidden to delete due to the remaining balance of the account');
            return;
        }
        ...
        if($r){
            $this->success('Successfully deleted',U('Member/index'));
            return;
        }else{
    ...
    public function ajaxCheckEmail($email){
        ...
        if(!checkEmail($email)){
            $data['status'] = 0;
            $data['msg'] = "Incorrect email format";
        }else{
            $M_member = M('Member');
            $where['email']  = $email;
    ...
    function ajaxCheckPhone($phone) {
        ...
        if(!checkMobile($phone)){
            $data['msg'] = "The phone number is incorrect!";
            $data['status'] = 0;
        }else{
            $M_member = M('Member');
            $where['phone']  = $phone;
    ...
    public function show(){
    	...
    	$member_id = I('member_id');
    	if(empty($member_id)){
    		$this->error('Parameter error',U('Member/index'));
    	}
    	$where['member_id'] = $member_id;
    ...
    public function updateMemberMoney(){
    	...
    	if(empty($member_id)||empty($member_id)){
    		$data['info']="Incomplete parameters";
    		$data['status']=0;
    	}
    	$where['member_id']=$member_id;
