...
class CurrencyController extends AdminController {
	...
	Public function add(){
		if(IS_POST){
			...
			if(!empty($cu)){
				$this->error("Currency name already exists");
			}
    ...
    public function del(){
    	if(empty($_GET['currency_id'])){
    		$this->error('Delete data does not exist');
    	}
    	$rs=M('Currency_user')->where('currency_id='.$_GET['currency_id'])->sum("num");
    
    	if($rs>0){
    		$this->error('The currency is still owned by the user and cannot be deleted.');
    	}
    ...
    public function  set_member_currencyForQianbao(){
    	$cuid=intval(I("cuid"));
    	if(empty($cuid)){
    		$this->error("Invalid currency parameter",U("Currency/index"));exit();
    	}
    	...
    	if(empty($currency)){
    		$this->error("Invalid currency",U("Currency/index"));exit();
    	}
    	if(IS_POST){
    		...
    		$num=I('num');//数量
    		if(empty($username)){
    			$this->error("Please enter user name");exit();
    		}
    		if(empty($num)||!is_numeric($num)){
    			$this->error("Please enter the number type");exit();
    		}
    		$member=M("Member")->where("email='$username'")->find();
    		if(empty($member)){
    			$this->error("Check no such person, please verify");exit();
    		}
    		
    		$qa=M("Qianbao_address")->where("user_id='{$member['member_id']}' and currency_id='{$cuid}'")->find();
    		if(empty($qa['qianbao_url'])){
    			$this->error("This user is not bound to the coin address and cannot transfer money.");exit();
    		}
    		//判断看这个钱包地址是否是真实地址
    		if(!$this->check_qianbao_address($qa['qianbao_url'],$currency)){
    		$this->error("The coin address is not a valid address");exit();
    		}
    		$num=floatval($num);
    		$data['fee']=0;//手续费
...
//     			M("Currency_user")->where("member_id='{$_SESSION['USER_KEY_ID']}' and currency_id='$cuid'")->setDec("num",$num);
    			$this->success("Transfer is successful, please be patient",U('Currency/index'));exit();
    		
    		}else{//失败提示
    			$this->error("Transfer failed");exit();
    		}
    	}
    ...
    public function savePrice(){
    	...
    	if(!is_numeric($_POST['price_up']) || !is_numeric($_POST['price_down'])){
    		$data['status'] = -2;
    		$data['info'] = 'Fill in must be a number';
    		$this->ajaxReturn($data);
    	}
