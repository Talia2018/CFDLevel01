...
class ManageController extends AdminController {
    ...
    private function auth(){
        $admin=$this->admin;
        if($admin['admin_id']!=1){
            $this->error('Only super administrators can access this page');exit();
        }
    }
    /**
     * 管理员列表
    ...
    public function addAdmin(){
        ...
        if(IS_POST){
            ...
            if(empty($username)||empty($pwd)){
                $this->error('Please complete the information');
            }
    ...
    public function saveNav(){
        ...
        if(I('post.admin_id')){
            ...
            $this->error('Incorrect operation');
        }
    }
    /**
     * 修改本账号密码
    ...
    public function pwdUpdate(){
        ...
        if(IS_POST){
            ...
            if($list['password'] != $old_pwd){
                $this->error('The original password you entered is incorrect.');
                return;
            }
            if(!checkPwd($_POST['password'])){
                $this->error('Verify password length between 6-20 characters');
                return;
            }
            ...
            if($r===true){
                ...
                $this->redirect('Login/loginout','',1,"<script>alert('Successfully Modified please login again')</script>");
                return;
            }
        }else{
            $this->assign('list',$list);
    ...
    public function delMember(){
        $this->auth();
        if(empty($_GET['admin_id'])){
            $this->error('The ID to be deleted does not exist');
        }
        if($_GET['admin_id']==1){
            $this->error('This ID cannot be deleted');
        }
        $admin = M('Admin');
        $r = $admin->delete($_GET['admin_id']);
        if($r){
            $this->success('successfully deleted',U('Manage/index'));
            return;
        }else{
