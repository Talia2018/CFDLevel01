...
<json id="json" ></json>
		...
		<tbody>
			...
			<?php if(empty($list)): ?><tr align="center" id="addAddress_tr">
			...
			<tr align="center">
				<td>地址</td>
				<td><input class="sum" id="new_label"  type="text" name="name" value="<?php echo ((isset($list["name"]) && ($list["name"] !== ""))?($list["name"]):''); ?>" disabled="disabled"></td>
				<td><input class="sum" id="wallet_address" style="width:300px;"  type="text" name="address" disabled="dosabled"  value="<?php echo ((isset($list["qianbao_url"]) && ($list["qianbao_url"] !== ""))?($list["qianbao_url"]):''); ?>"></td>
				<td>可用</td>
				
				<td><a href="<?php echo U('Pay/del_address',array('id'=>$list['id'],'cuid'=>$cuid['currency_id']));?>">删除</a></td>
			</tr><?php endif; ?>
		</tbody>
	    </table>
		  <script>	
        ...
        <div class="turns">
            ...
            <p>2. 每笔转出最大限额<?php echo ((isset($cuid["currency_all_tibi"]) && ($cuid["currency_all_tibi"] !== ""))?($cuid["currency_all_tibi"]):"0"); ?>个币。</p>
	...
	<table cellspacing="0" cellpadding="0" border="0" class="raise_list" align="center" style="border:1px solid #e1e1df;">
		...
		<?php if(is_array($tibi_list)): $i = 0; $__LIST__ = $tibi_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr id="btc_box" class="btc_new">
			...
			<td id="btc_number"><?php echo ($vo["num"]); ?></td>
			<td id="btc_fee"><?php echo ($vo["actual"]); ?></td>
			<td id="btc_created"><?php echo (date("Y-m-d H:i:s",$vo["add_time"])); ?></td>
			<td class="tableEnd" id="btc_status"><?php if(($vo["status"]) == "0"): ?>等待转出<?php else: ?>已经到账<?php endif; ?></td>
