    ...
    <div class="main_box">
        ...
        <div class="raise right clearfix">
            <div class="message">
				...
				<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$row): $mod = ($i % 2 );++$i;?><div class="mynews news01">
						<div class="andfrom yourattention">
							<p class="left news_name">
								<a href="
								<?php echo U('User/showSysTem',array('message_id'=>$row['message_id'],'message_all_id'=>$row['message_all_id']));?>
								">
									<?php if($row['status'] == 0 ): ?>【【<?php echo ($row['type_name']); ?>】<?php echo ($row['title']); ?>】<span style="color:#FF0000">未读</span>
										<?php else: ?>【【<?php echo ($row['type_name']); ?>】<?php echo ($row['title']); ?>】<?php endif; ?>
								</a>
							</p>
							<p class="right news_date"><?php echo (date("Y-m-d H:i:s ",$row['add_time'])); ?></p>
							<div class="clear">
							</div>
						</div>
