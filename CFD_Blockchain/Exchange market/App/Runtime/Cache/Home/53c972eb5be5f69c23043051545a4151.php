...
<div class="ybc_list">
	<div class="ybcoin clearfix">
		...
		<div class="right">
			<form action="<?php echo U('Entrust/manage');?>" method="post">
                <select id="coninname" name="currency">
                    <?php if(is_array($culist)): $i = 0; $__LIST__ = $culist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ((isset($vo['currency_id']) && ($vo['currency_id'] !== ""))?($vo['currency_id']):""); ?>"><?php echo (getCurrencynameByCurrency($vo['currency_id'])); ?>对<?php echo (getCurrencynameByCurrency($vo['trade_currency_id'])); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>    
                </select>
	 		...
	 		<thead>
				...
				<?php if(is_array($list)): foreach($list as $key=>$vo): ?><tr>
						...
						<td><?php echo (getCurrencynameByCurrency($vo["currency_id"])); ?></td>
							<?php if(($vo['type']) == "sell"): ?><td class="red"><?php else: ?><td style="color:#039904;"><?php endif; echo (fomatOrdersType($vo["type"])); ?></td>
						<td><?php echo (floatval($vo["num"])); ?></td>
						<td><?php echo (floatval($vo["price"])); ?></td>
						<td><?php echo (floatval($vo["trade_num"])); ?></td>
						<td><?php echo (floatval($vo['num']- $vo['trade_num'])); ?></td>
						<td><?php echo (formatOrdersStatus($vo["status"])); ?></td>
						<td><a href="javascript:void(0)"  onclick="cexiao(<?php echo ($vo["orders_id"]); ?>)">撤销</a></td>
