    ...
    <div class="newsarea_box">
      <div class="product left clearfix">
        <div style="border-bottom:1px solid #e2e2e2; padding-top:20px; position:relative;">
          ...
          <h2 style="line-height:24px; border:none;"><?php echo ($art_one["title"]); ?></h2>
          <p style=" position:absolute; right:40px; bottom:0px; color:#999;"><?php echo (date('Y-m-d H:i:s',$art_one["add_time"])); ?></p>
        </div>
        <div class="paragraph paragraph_news">
         <p><?php echo ($art_one["content"]); ?></p>
        </div>
      </div>
     <!--上面为显示的文章-->
      <div class="right">
        <div class="latest">
          <div class="investmentarea">
            <div class="focusnum1" style="border-bottom:1px solid #eee;">
              ...
              <p class="right"><a href="<?php echo U('Art/index',array('id'=>$art_one['position_id']));?>">查看更多</a></p>
              <div class="clear"></div>
            </div>
            
            <?php if(is_array($art_list)): $i = 0; $__LIST__ = $art_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vos): $mod = ($i % 2 );++$i;?><ul>
              <a href="<?php echo U('Art/details',array('id'=>$vos['article_id']));?>">
              <li>
                <p class="left" style="width:200px; line-height:18px;"><?php echo ($vos["title"]); ?></p>
                <p class=" right recruit" style="width:65px; margin-left:0;"><?php echo (date('Y-m-d',$vos["add_time"])); ?></p>
                <div class="clear"></div>
              </li>
              </a> 
