...
$(".sidebar-list li").children("a").on("click",function(){
    ...
    <div class="main-wrap">
        ...
        <div class="crumb-wrap">
            <div class="crumb-list"><i class="icon-font"></i><a href="<?php echo U('Index/index');?>">首页</a><span class="crumb-step">&gt;</span><span class="crumb-name">查看个人账户</span></div>
        </div>
            ...
            <div class="result-content">
                <table class="result-tab" width="100%">
                    ...
                    <?php if(is_array($info)): $i = 0; $__LIST__ = $info;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                            ...
                            <td><input  value="<?php echo ($vo['num']); ?>" type="text" id="num<?php echo ($vo['currency_id']); ?>"/></td>
							<td><input value="<?php echo ($vo['forzen_num']); ?>" type="text" id="forzen_num<?php echo ($vo['currency_id']); ?>"/></td>
                            <td><a onclick="update(<?php echo ($vo['currency_id']); ?>,num<?php echo ($vo['currency_id']); ?>)">修改</a></td>
...
<script>
	function update(_this,a){
	  ...
	  title: '修改'
		...
		var member_id="<?php echo ($info[0]['member_id']); ?>";
		var num=$("#num"+_this).val();
		var forzen_num=$("#forzen_num"+_this).val();
	  $.post("<?php echo U('Member/updateMemberMoney');?>",{currency_id:_this,member_id:member_id,num:num,forzen_num:forzen_num},function(data){
				   if(data['status'] == 1){
					   layer.msg(data['info']);
