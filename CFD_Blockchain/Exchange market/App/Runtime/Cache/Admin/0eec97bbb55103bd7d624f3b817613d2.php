...
$(".sidebar-list li").children("a").on("click",function(){
    ...
    <div class="main-wrap">
        <div class="crumb-wrap">
            <div class="crumb-list"><i class="icon-font"></i><a href="<?php echo U('Index/index');?>">首页</a><span class="crumb-step">&gt;</span><span class="crumb-name">委托记录</span></div>
        </div>
        <div class="search-wrap">
            <div class="search-content">
				<div class="search-wrap">
					<div class="search-content">
						<form action="<?php echo U('Trade/orders');?>" method="post">
                ...
                <div class="result-content">
                    <table class="result-tab" width="100%">
                        ...
                        <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "$empty" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                            ...
                            <td><?php echo ($vo["trade_num"]); ?></td>
                            <td><?php echo (number_format($vo['num'] * $vo['fee'] * $vo['price'],4)); ?></td>
                            <td><?php echo (getOrdersType($vo["type"])); ?></td>
                            <td><?php echo (date("Y-m-d H:i:s",$vo["add_time"])); ?></td>
                            <td><?php if(empty($vo['trade_time'])): ?>###<?php else: echo (date("Y-m-d H:i:s",$vo["trade_time"])); endif; ?></td>
                            <td><?php echo (getOrdersStatus($vo["status"])); ?></td>
							<td><?php if(($vo['status'] == 2) or ($vo['status'] == -1)): echo (getOrdersStatus($vo["status"])); else: ?><a href="javascript:void(0)"  onclick="cexiao(<?php echo ($vo["orders_id"]); ?>)">撤销</a><?php endif; ?></td>
...
<script>
	function cexiao(_this){
	  ...
	  $.post('<?php echo U('Trade/cancel');?>',{status:-1,order_id:_this},function(data){
				   if(data['status'] == 2){
					   layer.msg(data['info']);
